...
import { Search, DollarSign, TrendingUp, AlertCircle, CheckCircle, Clock, XCircle, Filter, Calendar, Building2, Phone, Mail, FileText } from 'lucide-react'
...
export default function VirtualFairPayments() {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true)
      const response = await api.get('/virtual-booth-applications')
  ...
  const calculateStats = (apps) => {
    ...
    const pending = apps.filter(a => a.paymentStatus === 'PENDING').length
  ...
  const filteredApplications = applications.filter(app => {
    const matchesSearch =
      ...
      app.fair?.title?.toLowerCase().includes(searchTerm.toLowerCase())
  ...
  const openPaymentModal = (app) => {
    ...
    setPaymentData({
      ...
      paidAmount: app.paidAmount || app.price || '',
      paidDate: app.paidDate ? new Date(app.paidDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
      paymentMethod: app.paymentMethod || 'Havale',
  ...
  const handlePaymentUpdate = async () => {
    try {
      await api.put(`/virtual-booth-applications/${selectedApp.id}/payment`, paymentData)
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Sanal Fuar Stant Ödemeleri</h1>
        <p className="text-gray-600">Sanal fuar stant başvurularının ödeme takibi</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600 mb-1">Toplam Başvuru</p>
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Firma veya fuar ara..."
              value={searchTerm}
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredApplications.length === 0 ? (
                ...
                filteredApplications.map((app) => {
                  const StatusIcon = paymentStatusConfig[app.paymentStatus]?.icon || AlertCircle
                  return (
                    <tr key={app.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-gray-900">{app.companyName}</span>
                          <span className="text-xs text-gray-500">{app.fair?.title || '-'}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="text-sm text-gray-900">{app.contactName}</span>
                      ...
                      <td className="px-6 py-4">
                        <span className="text-sm text-gray-900">{app.boothType?.name || '-'}</span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          ...
                          {app.paidAmount && (
                            <span className="text-xs text-green-600">
                              Ödenen: {formatCurrency(app.paidAmount)}
                            </span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${
                            paymentStatusConfig[app.paymentStatus]?.color || 'bg-gray-100 text-gray-800'
                          }`}>
                            <StatusIcon className="w-3 h-3" />
                            {paymentStatusConfig[app.paymentStatus]?.label || app.paymentStatus}
                          </span>
                        </div>
                        {app.paidDate && (
                          <div className="flex items-center gap-1 text-xs text-gray-500 mt-1">
                            <Calendar className="w-3 h-3" />
                            {formatDate(app.paidDate)}
                          </div>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${
                          applicationStatusConfig[app.status]?.color || 'bg-gray-100 text-gray-800'
                        }`}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-600 mt-1">{selectedApp.companyName} - {selectedApp.fair?.title}</p>
            </div>

            <div className="p-6 space-y-4">
              {/* Başvuru Bilgileri */}
              <div className="bg-gray-50 p-4 rounded-lg">
                <h3 className="font-semibold text-gray-900 mb-3">Başvuru Bilgileri</h3>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600">Firma:</span>
                    <span className="ml-2 font-medium">{selectedApp.companyName}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Yetkili:</span>
                    <span className="ml-2 font-medium">{selectedApp.contactName}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Stant Tipi:</span>
                    <span className="ml-2 font-medium">{selectedApp.boothType?.name || '-'}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Tutar:</span>
                    <span className="ml-2 font-medium">{formatCurrency(selectedApp.price)}</span>
                  </div>
                </div>
              </div>
